...
- **Framed Ledger Action Buttons:** Refactored the ledger balances card Row 2 layout to frame the Portal launcher and Add Transaction buttons inside a single, unified flex-container. Modeled after the homepage tabs switcher, the container uses a slate grey background, inner shadows, and a 2xl corner radius, displaying the Portal link as a clean white inactive tab and the Add Tx trigger as an active cobalt blue tab.
- **Aligned Controls Switcher Container Layout:** Standardized the homepage Controls Card structure to adopt the exact same three-row nesting architecture and vertical layout parameters as the detailed account Ledger Card. Stacking Title block in Row 1, View Switchers in Row 2, and dynamic Search inputs in Row 3 inside an identical h-full justify-between wrapper locks matching element bounds to the exact vertical pixel with zero toggle jitter.
- **Planned Transactions Cockpit Typography Upgrade:** Upscaled the font sizes, weights, and letter tracking metrics across both the desktop Planned Transactions widget and the mobile slide-up modal drawer. Payee headings now pop at text-sm font-black, metadata labels are styled with text-xs tracking-widest, value amounts are set to text-sm font-black, and "Post" buttons use a premium, tap-friendly rounded-xl visual presence to complete the system-wide visual standardization.
...
- **Locked Cockpit Vertical Parity:** Synchronized the vertical height footprint of Row 1 inside the ledger balances card to match the homepage switcher title block. This ensures flex-wrap whitespace distribution is identical across both layouts, completely eliminating the remaining 4px shifting of buttons and search inputs when toggling panels.
